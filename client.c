...
int npsd_check_valid_filelist(char* filelist) {
    // CheckÃ‚Â there is at least one file in the file list
...
int check_valid_extensions(char* extensions) {
    // CheckÃ‚Â at least one extension is present in the extensions list
   ...
   //Intilizing necessay variables
    ...
    while (1) {
      ...
      //  *memset copies the character c (an unsigned char) to the first n characters of the string pointed to, by the argument str.
        ...
        if (token == NULL) {
            valid_syntax = 0;
        } else if (strcmp(token, "getfn") == 0) { //Function -- with filesrch command
            char* fname = strtok(NULL, " ");
            if (fname == NULL) {
                valid_syntax = 0;
           
                } else {
            sprintf(command, "getfn %s", fname);
        }
        ...
        if (size1 == NULL || size2 == NULL || atoi(size1) < 0 || atoi(size2) < 0) {
            ...
            if (unzip != NULL && strcmp(unzip, "-u") == 0) {
                sprintf(command, "getfz %s %s -u", size1, size2);
            } else {
                sprintf(command, "getfz %s %s", size1, size2);
            }
        }
    } 
    /*
        ...
        if (date1 == NULL || date2 == NULL || !npsd_check_valid_date(date1) || !npsd_check_valid_date(date2)) {
            ...
            char* unzip = strtok(NULL, " ");
            if (unzip != NULL && strcmp(unzip, "-u") == 0) {
                sprintf(command, "getfdb %s %s -u", date1, date2);
            } else {
                sprintf(command, "getfdb %s %s", date1, date2);
            }
        }
    } 
    */
    else if (strcmp(token, "getfdb") == 0) {  // Function with getfdb command
    char* date_str1 = strtok(NULL, " ");
    if (date_str1 == NULL || !npsd_check_valid_date(date_str1)) {
        valid_syntax = 0;
        printf("Enter a valid date.\n");
    } else {
        sprintf(command, "getfdb %s", date_str1);
    }
}

else if (strcmp(token, "getfda") == 0) {  // Function with getfda command
    char* date_str2 = strtok(NULL, " ");
    if (date_str2 == NULL || !npsd_check_valid_date(date_str2)) {
        valid_syntax = 0;
        printf("Enter a valid date.\n");
    } else {
        sprintf(command, "getfda %s", date_str2);
    }
}
